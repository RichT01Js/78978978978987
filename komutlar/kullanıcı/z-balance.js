  ...
  if (!hesapdurumu) {
    if (args[0])
      return message.reply(
        `Kullanıcının hesabı bulunmamakta.`
      );
    message.reply(
      `İlk olarak hesap oluşturmalısın. ${client.ekoayarlar.botunuzunprefixi}acc <Hesap İsmi>`
    );
  } else {
    if (hesapdurumu) {
        const embedczdn = new Discord.MessageEmbed()
          ...
          .setDescription(
            `Hesap Adı: ${hesapismi ? hesapismi : 'Bilinmiyor.'}\n Bakiye: **${formattedBakiye} ${client.ekoayarlar.parabirimi}**`
          );
